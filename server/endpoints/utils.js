...
function getGitVersion() {
  if (process.env.UNIVERSAL_LLM_RUNTIME === "docker") return "--";
  try {
 ...
 * as well as build features in UniversalLLM directly for a specific model or capabilities.
 *
 * Disable with  {@link https://github.com/ismailubts/UniversalLLM?tab=readme-ov-file#telemetry--privacy|Disable Telemetry}
...
/**
 * Returns the user agent for the UniversalLLM deployment.
 * @returns {string} The user agent.
 */
function getUniversalLLMUserAgent() {
  const version = getDeploymentVersion() || "unknown";
  return `UniversalLLM/${version}`;
}
