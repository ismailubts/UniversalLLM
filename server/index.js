...
if (!process.env.ENABLE_HTTPS) {
  ...
  (async () => {
    const flag = await prisma.system_settings.findUnique({
      where: { label: "chat_mode_default_migrated_v1" },
    });
    if (!flag) {
      await prisma.workspaces.updateMany({
        where: { chatMode: "automatic" },
        data: { chatMode: "chat" },
      });
      await prisma.system_settings.create({
        data: { label: "chat_mode_default_migrated_v1", value: "true" },
      });
    }
  })().catch((e) =>
    console.error("Workspace chatMode migration failed:", e.message)
  );
