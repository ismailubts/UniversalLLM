...
/**
 * An interface to the UniversalLLM Community Hub external API.
 */
const LEGACY_IMPORT_PREFIX = "allm-community-id";

const CommunityHub = {
  importPrefix: "ullm-community-id",
  apiBase:
    process.env.COMMUNITY_HUB_API_BASE ||
    (process.env.NODE_ENV === "development"
      ? "http://127.0.0.1:5001/universalllm-hub/us-central1/external/v1"
      : ""),
  ...
  validateImportId: function (importId) {
    const validPrefixes = [this.importPrefix, LEGACY_IMPORT_PREFIX];
    if (
      !importId ||
      !validPrefixes.some((prefix) => importId.startsWith(prefix)) ||
   ...
   * so we can unzip and import them to the UniversalLLM instance.
  ...
  /**
   * Apply an item to the UniversalLLM instance. Used for simple items like slash commands and system prompts.
  ...
  /**
   * Import a bundle item to the UniversalLLM instance by downloading the zip file and importing it.
