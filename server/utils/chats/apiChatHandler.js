 ...
 * the mime type for documents is `application/universalllm-document` - anything else is assumed to be an image.
...
async function processDocumentAttachments(attachments = []) {
  ...
  for (const attachment of attachments) {
    if (
      ...
      attachment.mime &&
      attachment.mime.toLowerCase() === "application/universalllm-document"
    )
...
async function chatSync({
  ...
  const uuid = uuidv4();
  const chatMode = mode ?? workspace?.chatMode ?? "chat";
