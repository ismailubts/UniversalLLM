 ...
 *
 * When `UNIVERSALLLM_FETCH_TIMEOUT` is set (milliseconds), both the undici
...
function patchSdkTimeouts() {
  const envDefinedTimeout = process.env.UNIVERSALLLM_FETCH_TIMEOUT;
  const envDefinedMaxRetries = process.env.UNIVERSALLLM_MAX_RETRIES;
  ...
  if (envDefinedTimeout) {
    ...
    if (!Number.isFinite(parsed) || parsed <= 0) {
      console.warn(
        `${LOG_PREFIX} UNIVERSALLLM_FETCH_TIMEOUT="${envDefinedTimeout}" is not a valid positive integer — using default ${DEFAULT_TIMEOUT_MS}ms.`
      );
    } else {
      timeoutMs = parsed;
    }
  ...
  if (envDefinedMaxRetries) {
    ...
    if (!Number.isFinite(parsed) || parsed < 0) {
      console.warn(
        `${LOG_PREFIX} UNIVERSALLLM_MAX_RETRIES="${envDefinedMaxRetries}" is not a valid non-negative integer — using default ${DEFAULT_MAX_RETRIES}.`
      );
    } else {
      maxRetries = parsed;
    }
