...
async function validDockerizedUrl(input = "") {
  if (process.env.UNIVERSAL_LLM_RUNTIME !== "docker") return null;
  ...
  try {
    ...
    if (isPortAvailableFromDocker)
      return "Port is not running a reachable service on loopback address from inside the UniversalLLM container. Please use host.docker.internal (for linux use 172.17.0.1), a real machine ip, or domain to connect to your service.";
  } catch (error) {
...
function dumpENV() {
  ...
  const protectedKeys = [
    ...
    "UNIVERSALLLM_FETCH_TIMEOUT",
    "UNIVERSALLLM_MAX_RETRIES",
