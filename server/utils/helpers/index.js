 ...
 * @notice Use resolveProviderConnector instead as this function DOES NOT handle the universalllm-router provider.
 * You should only use this function if you are absolutely sure you are not using the universalllm-router provider ever in your code.
...
function getLLMProvider({ provider = null, model = null } = {}) {
  ...
  switch (LLMSelection) {
    ...
    case "universalllm-router":
      // Model router is handled separately in stream.js via UniversalLLMModelRouter.
      // This case should not be hit directly - if it is, throw a descriptive error.
      throw new Error(
        "universalllm-router provider must be resolved via UniversalLLMModelRouter class, not getLLMProvider directly."
      );
    default:
      throw new Error(
...
function getLLMProviderClass({ provider = null } = {}) {
  switch (provider) {
    ...
    case "cerebras":
      ...
      return CerebrasLLM;
    case "universalllm-router":
      const { UniversalLLMModelRouter } = require("../AiProviders/modelRouter");
      return UniversalLLMModelRouter;
    default:
      return null;
  }
}
 ...
 * handling the universalllm-router provider transparently. Callers get back
...
async function resolveProviderConnector({
  ...
  if (effectiveProvider !== "universalllm-router") {
    return {
  ...
  const router = new UniversalLLMModelRouter(routerWorkspace);
