# How to use Dockerized UniversalLLM

Use the Dockerized version of UniversalLLM for a much faster and complete startup of UniversalLLM.

### Minimum Requirements

> [!TIP]
> Running UniversalLLM on AWS/GCP/Azure?
...
\*UniversalLLM by default uses a built-in vector database powered by [LanceDB](https://github.com/lancedb/lancedb)

\*UniversalLLM by default embeds text on instance privately [Learn More](../server/storage/models/README.md)

## Recommend way to run dockerized UniversalLLM!
...
> the docker container using UniversalLLM as `localhost:xxxx` will not resolve for the host system.
>
...
```shell
docker pull ismailubts/universalllm
```

<table>
<tr>
<th colspan="2">Mount the storage locally and run UniversalLLM in Docker</th>
</tr>
<tr>
<td>
  Linux/MacOs
...
```shell
export STORAGE_LOCATION=$HOME/universalllm && \
...
$env:STORAGE_LOCATION="$HOME\Documents\universalllm"; `
...
services:
  universalllm:
    image: ismailubts/universalllm
    container_name: universalllm
    ports:
    - "3001:3001"
    cap_add:
      - SYS_ADMIN
    ...
    volumes:
      - universalllm_storage:/app/server/storage
    restart: always

volumes:
  universalllm_storage:
    driver: local
    driver_opts:
      type: none
      o: bind
...
Go to `http://localhost:3001` and you are now using UniversalLLM! All your data and progress will persist between
...
- `git clone` this repo and `cd UniversalLLM` to get to the root directory.
- `touch server/storage/universalllm.db` to create empty SQLite DB file.
- `cd docker/`
...
The integrations below are templates or tooling built by the community to make running the docker experience of UniversalLLM easier.

### Use the Midori AI Subsystem to Manage UniversalLLM

Follow the setup found on [Midori AI Subsystem Site](https://io.midori-ai.xyz/subsystem/manager/) for your host OS
After setting that up install the UniversalLLM docker backend to the Midori AI Subsystem.
...
Then in docker you need to replace that localhost part with `host.docker.internal`. For example, if running Ollama on the host machine, bound to http://127.0.0.1:11434 you should put `http://host.docker.internal:11434` into the connection URL in UniversalLLM.
