...
if ! whoami &> /dev/null 2>&1; then
  if [ -w /etc/passwd ]; then
    echo "universalllm:x:$(id -u):0:UniversalLLM User:/app:/bin/bash" >> /etc/passwd
  fi
fi
export HOME=/app
...
if [ -z "$STORAGE_DIR" ]; then
    ...
    echo "https://github.com/ismailubts/UniversalLLM/blob/master/docker/HOW_TO_USE_DOCKER.md"
    echo ""
    echo "⚠️  ⚠️  ⚠️  WARNING: STORAGE_DIR environment variable is not set! ⚠️  ⚠️  ⚠️"
    echo "================================================================"
fi
