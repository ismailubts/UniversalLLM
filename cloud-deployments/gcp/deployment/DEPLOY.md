...
2. After successful login, Run the following command to create a deployment using the Deployment Manager CLI:
  ...
  gcloud deployment-manager deployments create universal-llm-deployment --config gcp/deployment/gcp_deploy_universal_llm.yaml
...
```
gcloud compute instances get-serial-port-output universal-llm-instance 
```

ssh into the instance

```
gcloud compute ssh universal-llm-instance 
```

Delete the deployment
```
gcloud deployment-manager deployments delete universal-llm-deployment 
```
